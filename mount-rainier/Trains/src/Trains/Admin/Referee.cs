...
namespace Trains.Admin
{
    ...
    public class Referee
    {
        ...
        public Referee(Dictionary<Color, uint> startingCards, uint railsPerPlayer, uint cardsPerPlayer, TrainsMap map, List<IPlayer> players, Deck? deck = null)
        {
            ...
            if (deck is null)
            {
                deck = new Deck(startingCards, map.GetAllFeasibleDestinations().ToHashSet());
            }
        ...
        public Referee(Dictionary<Color, uint> startingCards, uint railsPerPlayer, uint cardsPerPlayer, TrainsMap map, List<IPlayer> players, Random random) :
            this(startingCards, Constants.setupPlayerRailsCount, Constants.setupPlayerCardsCount, map, players, new Deck(startingCards, map.GetAllFeasibleDestinations().ToHashSet(), random))
        { }
        ...
        void SetUpPlayers()
        {
            foreach (var player in new List<IPlayer>(_gameState.Players))
            {
                ...
                IImmutableSet<Destination> chosenDests = new HashSet<Destination>().ToImmutableHashSet(); // make empty for if check
                if (!WrapPlayerCall(() => player.Setup(_map, Constants.setupPlayerRailsCount, _gameState.Cards.DrawCards(Constants.setupPlayerCardsCount)), player)
                    || !WrapPlayerCall(() => chosenDests = player.ChooseDestinations(givenDests), player)
                    || chosenDests.Count > Constants.destinationsPerPlayer)
                {
                    continue;
                }
                // check the response is valid by looping to make it size agnostic, we checked validity of size above
                foreach (Destination dest in chosenDests)
                {
                ...
                _gameState.Cards.Destinations.RemoveWhere(d => chosenDests.Contains(d));
                _gameState.PlayerInventories[player].Destinations = new HashSet<Destination>(chosenDests);
            }
        }
        ...
        void HandleTurn(PlayerResponse response, IPlayer player)
        {
            ...
            {
                var dealtCards = _gameState.Cards.DrawCards(_cardsPerPlayer);
