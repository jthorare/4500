...
namespace Trains.Admin
{
    ...
    public sealed class RefereeGameState : AbstractGameState, ICloneable
    {
        ...
        public PlayerGameState GetPlayerGameState(IPlayer player)
        {
            ...
            foreach (var con in ClaimedConnections)
            {
                claimedConnections.Add(con.Value.ToImmutableHashSet());
            }

            return new PlayerGameState(Phase, NumberOfPlayers, claimedConnections.ToImmutableList(),
        ...
        private IPlayer? PlayerWithLongestPath()
        {
        ...
        public Dictionary<IPlayer, int?> GetRankings()
        {
            ...
            IPlayer playerWithLongestPath = PlayerWithLongestPath();

            foreach (var player in Players)
            {
                rankings.Add(player, GetScore(player) + (player == playerWithLongestPath ? 20 : 0));
            }
        ...
        public object Clone()
        {
            ...
            {
                playersInvs.Add(entry.Key, new PlayerInventory(new Dictionary<Color, uint>(entry.Value.ColoredCards), entry.Value.RailCards,
            ...
            return new RefereeGameState(Phase, NumberOfPlayers, new List<IPlayer>(Players),
                ...
                new Deck(new Dictionary<Color, uint>(Cards.ColoredCards), new HashSet<Destination>(Cards.Destinations)), KickedPlayers);
        }
        ///<inheritdoc/>
