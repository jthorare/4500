...
namespace Trains.Common.Map
{
    ...
    public sealed partial class TrainsMap
    {
        ...
        private bool Equals(TrainsMap other)
        {
            var sameConnections = _connections.SequenceEqual(other._connections);
            var sameLocations = true;
            foreach (var (name, location) in _locations)
            {
                if (!other._locations.ContainsKey(name) || other._locations[name] != location) sameLocations = false;
            }

            return sameConnections && sameLocations;
        }

        public override bool Equals(object? obj)
        {
            return ReferenceEquals(this, obj) || obj is TrainsMap other && Equals(other);
        }

        public override int GetHashCode()
        {
            return HashCode.Combine(_connections, _locations);
        }

        public static bool operator ==(TrainsMap? left, TrainsMap? right)
        {
