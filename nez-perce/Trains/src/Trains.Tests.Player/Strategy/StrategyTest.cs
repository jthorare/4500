...
namespace Trains.Tests.Player.Strategy
{
    ...
    public class StrategyTest
    {
        ...
        public void Initialize()
        {
            ...
            _noCardsMaxRails = new PlayerInventory(new Cards(), Constants.setupPlayerRailsCount, new HashSet<Destination>());
            _maxCardsMaxRails = new PlayerInventory(Constants.defaultDeck, Constants.setupPlayerRailsCount, new HashSet<Destination>());
            _pgs_noCards = new PlayerGameState(GamePhase.InProgress, int.MaxValue, new HashSet<Connection>().ToImmutableHashSet(),
               new List<ImmutableHashSet<Connection>>().ToImmutableList(), map.Connections.ToImmutableHashSet(), _noCardsMaxRails);
            _pgs_MaxCards = new PlayerGameState(GamePhase.InProgress, int.MaxValue, new HashSet<Connection>().ToImmutableHashSet(),
                new List<ImmutableHashSet<Connection>>().ToImmutableList(), map.Connections.ToImmutableHashSet(), _maxCardsMaxRails);
            _pgs_NoConnections = new PlayerGameState(GamePhase.InProgress, int.MaxValue, new HashSet<Connection>().ToImmutableHashSet(),
                new List<ImmutableHashSet<Connection>>().ToImmutableList(), map.Connections.ToImmutableHashSet(), _maxCardsMaxRails);
        }
        ...
        public void PlayTurnTest()
        {
            ...
            var expected = PlayerResponse.ClaimConnection(_connections.Where(x => x.Locations.Select(x => x.Name).ToHashSet().SetEquals(new HashSet<string>() { "Honolulu", "Anchorage" })).FirstOrDefault());
